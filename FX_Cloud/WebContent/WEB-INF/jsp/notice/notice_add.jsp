...
$(function(){
	$(".newVer").show();
	$(".newNews").hide();
	$("#chooseTyp").on("change",function runc(enent){
		$(".newVer").toggle();
		$(".newNews").toggle();
	});
});
...
<div class="right">
 <p class="right_position"><img src="<%=request.getContextPath()%>/resources/image/position.png" /><strong>当前位置:</strong>&nbsp;首页&nbsp;》&nbsp;消息管理&nbsp;》&nbsp;消息添加</p>
 <div class="search">
   <p class="system_page_title"><span class="mouse_over"><strong>添加</strong></span></p>
   <ul class="add_content">
     <table class="add_table">
     	<tr>
     	<td width="10%" class="add_title">选择发布类型:</td>
     	<td>
      	<select name="choose" class="text_class_more" id="chooseTyp">
      		<option value="0">发布新版本</option>
      		<option value="1">发布新消息</option>
      	</select>
      	&nbsp;${messageWarn}${messageVerTyp}${messageTheme}
      	</td>
      	</tr>
        <tr class="newVer">
         <td class="add_title">新版本软件型号:</td>
         <td><sf:input path="routerVer" class="text_class_more" name="routerVer" style="width:200px" /><sf:errors path="routerVer"/><span style="margin-left:10px;color:red;">版本型号格式例如：V1.0.1</span></td>
       </tr>
		<tr class="newVer">
       ...
       <tr class="newVer">
         ...
         <td><sf:input path="verDesc" class="text_class_more" name="verDesc" style="width:200px" /><sf:errors path="verDesc"/></td>
       </tr>
        <tr class="newVer">
