...
dsy.add("0_23_0", ["徐汇区","杨浦区","黄浦区","虹口区","浦东新区","静安区","长宁区","普陀区","闸北区","宝山区","闵行区","松江区","青浦区","嘉定区","金山区","奉贤区"]);
dsy.add("0_23_1", ["崇明县"]);
...
dsy.add("0_25_0", ["和平区", "南开区", "河西区","河东区","河北区","红桥区","东丽区","西青区","津南区","北辰区","武清区","宝坻区","滨海新区"]);
dsy.add("0_25_1", ["蓟县"]);
dsy.add("0_25_2", ["静海县"]);
dsy.add("0_25_3", ["宁河县"]);
...
function selMod()
{
	var model_ob=document.getElementById("m1");
	loadXMLDoc("/FX_Cloud/notice/notice_check/"+model_ob.options[model_ob.selectedIndex].value);
...
function init_city(){
	var str = document.forms[0].area.value;
	var strs= new Array(); 

	strs=str.split("_"); 
	document.getElementById("s1").selectedIndex = strs[1];
	change(1);
	document.getElementById("s2").selectedIndex = strs[2];
	change(2);
	document.getElementById("s3").selectedIndex = strs[3];
}
function submitData()
{
	...
	cf.area.value = 0+"_"+a+"_"+b+"_"+c;
}
function cancelUpdate(){
	top.location.href = "<%=request.getContextPath()%>/home/index/" + document.forms[0].id.value;
}
</script>
</head>
  ...
  <tr>
    ...
    <td colspan="3" rowspan="2"  class="re_noticeTd1">
        <table width="540" height="360" border="0" cellpadding="0" cellspacing="0" class="re_word">
          ...
          <tr>
            <td width="33">&nbsp;</td>
            <td width="137">电子邮箱：</td>
            <td width="19">&nbsp;</td>
            <td colspan="2"><input type="hidden" value="${user.id}" name="id"/><sf:hidden path="mailAddress"/>&nbsp;${user.mailAddress}</td>
          </tr>
          <tr>
            <td>&nbsp;</td>
            <td>用户名/姓名：</td>
            <td>&nbsp;</td>
            <td colspan="2"><sf:input type="text" path="username"/>&nbsp;${message}</td>
          </tr>
          <tr>
            <td>&nbsp;</td>
            <td>区&nbsp;&nbsp;&nbsp;号：</td>
            <td>&nbsp;</td>
            <td colspan="2"><select name="province" id="s1"><option>省份</option></select>
				<select name="city" id="s2"><option>地级市</option></select>
				<select name="county" id="s3"><option>市、县级市、县</option></select></td>
          </tr>
          <tr>
            <td>&nbsp;</td>
            <td>手机号（可选）：</td>
            <td>&nbsp;</td>
            <td colspan="2"><sf:input type="text" path="Phone"/>&nbsp;</td>
          </tr>
          <tr>
            <td>&nbsp;</td>
            <td>邮编（可选）：</td>
            <td>&nbsp;</td>
            <td colspan="2"><sf:input type="text" path="ZipCode"/>&nbsp;</td>
          </tr>
          <tr>
