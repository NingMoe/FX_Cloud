$(function(){
	$( "#publishStartDate" ).datetimebox({
		showSeconds:false
	});
	$( "#publishEndDate" ).datetimebox({
		showSeconds:false
	});
	...
	$("#addBtn").click(function(){
		if(validate.valid()) {
			...
			for(var i=0;i<3;i++)
			{ 		
				strselect+=("_"+(document.getElementById(s[i]).selectedIndex));
			}
			$("#publishRange").val(strselect);
			$("#addForm").submit();
			$(this).attr("disabled");
		}
	});
